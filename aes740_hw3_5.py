...
model_qv = np.array(netCDF4.Dataset('C:/Users/jmayhall/Downloads/aes740_hw3/cm1out_diff.nc').variables.get('qv'))[:, :, 0, :]
qc = np.array(netCDF4.Dataset('C:/Users/jmayhall/Downloads/aes740_hw3/cm1out_diff.nc').variables.get('qc'))[:, :, 0, :]
qr = np.array(netCDF4.Dataset('C:/Users/jmayhall/Downloads/aes740_hw3/cm1out_diff.nc').variables.get('qr'))[:, :, 0, :]
ql = qc + qr

"""Source: https://stackoverflow.com/questions/76277163/plotting-the-parcel-virtual-temp-profile-in-metpy-1-5"""
below_lcl = p > lcl_pressure
parcel = mpcalc.relative_humidity_from_dewpoint(prof, Td)
parcel_mixing_ratio = np.where(below_lcl, mpcalc.saturation_mixing_ratio(p, Td),
                               mpcalc.saturation_mixing_ratio(p, T))[:-1]
difference = np.subtract(parcel_mixing_ratio.magnitude, mpcalc.saturation_mixing_ratio(p, Td)[:-1])
