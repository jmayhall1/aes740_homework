...
data = pd.read_csv('gaudet2000sounding.txt', header=None, sep='\s+')
data.columns = ['pressure', 'temperature', 'dewpoint', 'direction', 'speed']
u, v = mpcalc.wind_components(np.array(data.speed) * units('kts'),
                              np.multiply(np.divide(np.array(data.direction), 180), np.pi))
T, Td, p = (np.array(data.temperature) * units('degC'), np.array(data.dewpoint) * units('degC'),
            np.array(data.pressure) * units('hPa'))
z = mpcalc.pressure_to_height_std(p)

print(f'GC {getcape(p, T, Td, adiabat='rl')}')
